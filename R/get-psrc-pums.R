...
get_psrc_pums <- function(span, dyear, target_var, group_var=NULL, bin_defs=NULL){
  ...
  pums_vars     <- tidycensus::pums_variables %>% data.table::setDT() %>% dplyr::filter(year==dyear & survey==paste0("acs", span) )   # Retrieve variable definitions
  tbl_ref       <- pums_vars%>% dplyr::filter(var_code==target_var) %>% dplyr::select(level) %>% dplyr::distinct()                                  # Table corresponding to unit of analysis (for rep weights)

  if(!is.null(group_var)){
  key_ref       <- pums_vars%>% dplyr::filter(var_code==group_var) %>% dplyr::select(level) %>% dplyr::distinct()
  # Table corresponding to grouping variable (for join)
...
# To match join
  ...
  dt %<>% data.table::setDF() %>%
    srvyr::as_survey_rep(variables=tidyselect::all_of(varlist),                                                # Create srvyr object with replication weights for MOE
                  weights=tidyselect::all_of(rwgt_ref),
                  repweights=tidyselect::all_of(rw),
                  combined_weights=TRUE,
                  mse=TRUE,
                  type="other",
                  scale=4/80,
                  rscale=length(tidyselect::all_of(rw)))
  return(dt)
}
...
psrc_pums_stat <- function(stat_type, geo_scale, span, dyear, target_var, group_var=NULL, bin_defs=NULL){
  result_name <- rlang::sym(stat_type)
  ...
  groupvar_label <- paste0(group_var,"_label")
  df %<>% dplyr::group_by(.data[[groupvar_label]])}
  ...
  rs <- dplyr::summarize(df, !!result_name:=(as.function(!!srvyrf_name)(
    ...
    dplyr::mutate(!!rlang::sym(moe_name):=!!rlang::sym(se_name) * 1.645) %>% data.table::setDT()
