...
To do the summary, first you need to aggregate the data provided by households spending over 30% based on the available fields-Less than 10.0 percent|10.0 to 14.9 percent|15.0 to 19.9 percent|20.0 to 24.9 percent|25.0 to 29.9 percent. This provides the numerator of your calculation.
...
#### Step 2. Add a grouping for more than 30% of income going to housing cost
```{r }
...
### Step 3: Aggregate by the rent burden group and the tract id, calculate sums, and the moe_sum. 
...
acs_data_group <- acs_data %>% 
  ...
  dplyr::summarise(estimate=sum(estimate), moe=tidycensus::moe_sum(estimate=estimate, moe=moe))


```

###Step 4: Data wrangling to get the aggregated data in the right shape, pivot, subtract, divide.
```{r, include=FALSE}
...
### Step 5: Calculate margins of error on the transformed data. 
...
acs_data_group<- acs_data_group%>%
  ...
  mutate(moe_perc_cost_burdened=moe_prop(num=estimate_cost_burdened,denom=estimate_new_total, moe_num=moe_cost_burdened, moe_denom=moe_new_total))
...
acs_data_final<-reliability_calcs(acs_data_final, estimate='estimate_perc_cost_burdened', moe='moe_perc_cost_burdened')

head(acs_data_final%>%select(GEOID, estimate_perc_cost_burdened, moe_perc_cost_burdened))
```

Now the data is ready to be joined to the tract map.
