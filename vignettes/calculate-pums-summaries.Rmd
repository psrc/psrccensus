...
get_psrc_pums(span = 5,                               # Denoting ACS 5-year estimates; 1-year also available
              dyear = 2019,                           # Last data year of span
              level = "p",                            # Unit of analysis == person; "h" used for household
              vars = "HINCP") %>%                     # PUMS variable name for household income
  psrc_pums_median(target_var = "HINCP")              # Only 2nd argument is necessary (chaining)
```
...
ni <- get_psrc_pums(5, 2019, "h", c("HINCP","TEN"))   # PUMS household income & housing tenure variables
...
psrc_pums_count(ni, group_var="TEN")                  # If omitting target_var argument, group_var must be labeled
```
...
psrc_pums_summary(ni, "HINCP", "TEN")
```
