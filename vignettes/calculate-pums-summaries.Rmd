...
The second step involves calculating summary statistics from the result of the prior step. There are five separate functions, one for each of four statistics, and a summary function that combines all four in one call. Each function returns both county and regional subtotals (by group, if grouping variable/s are specified), and the overall regional total (without grouping).
...
PUMS data is available for years 1996-present; get_psrc_pums() must be called separately for each data year. Note, some variables have changed name or datatype over time: consult the data directories mentioned above to verify the variable code for the data years pertinent to your analysis. 
