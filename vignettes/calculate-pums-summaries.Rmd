...
county_pums_median(ni, "HINCP", "TEN") 

```

The count functions tabulate the number people or households by grouping category. The second argument is the (optional) grouping variable, as the primary variable is implicit. 
```{r county count by category, message=FALSE, eval=FALSE}
county_pums_count(ni, "TEN")
```
