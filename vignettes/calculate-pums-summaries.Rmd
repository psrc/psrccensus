...
The first step consists of retrieving the data of interest via the **get_psrc_pums()** function. The arguments are:
  ...
  * **vars**  - one or more PUMS variables (if multiple, as a list), quoted exactly, in uppercase
...
x <- get_psrc_pums(span = 5,                          # Denoting ACS 5-year estimates; 1-year also available
                   ...
                   vars = c("HINCP","TEN"))           # PUMS household income & housing tenure variables; using tenure later. You can choose as many variables as you need.
psrc_pums_median(ni, target_var = "HINCP")            # Median Household income (without regard to tenure)
```

To calculate by a subgroup category (here, housing tenure--e.g. own vs. rent), use the optional grouping variable argument. As is typical, function argument names can be omitted if arguments are provided in strict order.
...
Although psrccensus allows you to specify multiple grouping variables--in addition to county--you'll need to keep in mind that PUMS is a relatively small-sample. If there are no respondents for one of your category combinations, the srvyr statistical functions will return an error; if there are few respondents, the margin of error may be so large as to render the estimate itself almost useless. If this occurs, you may want to consider either switching from the 1-year to 5-year span (for a larger sample), or potentially analyze dimensions of the data separately rather than in combination. If a grouping combination does not exist for one of the counties, use the `incl_counties=FALSE` option to return only the regional estimate.

PUMS data is available for years 2000-present; get_psrc_pums() must be called separately for each data year. Note, some variables have changed name or datatype over time: consult the data directories mentioned above to verify the variable code for the data years pertinent to your analysis. 

The current default source is via ftp download (there is an API option, `ftp=FALSE`, but the API often returns an internal error, particularly between 2pm-7pm).
